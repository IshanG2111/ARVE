    ...
    async def execute_ingestion_pipeline(
        ...
        try:
            ...
            if user_access_token and user_access_token.startswith("ghp_"):
                try:
                    tarball_bytes = await client.download_repository_tarball(repo.owner, repo.name, run.commit_sha)
                    import tarfile
                    import io
                        ...
                        async with semaphore:
                            try:
                                f_data = await client.get_file_content(
                                    repo.owner, repo.name, e_entry.path, run.commit_sha, client=http_pool
                                )
            ...
            self.db.query(RepositoryFile).filter(RepositoryFile.repository_id == repo.id).delete(synchronize_session=False)
            ...
            repo.frameworks = frameworks_str
            repo.package_manager = package_manager
