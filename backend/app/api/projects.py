...
async def _fetch_github_repository(current_user: User, repository) -> dict:
    """Verify the selected GitHub repository using the authenticated user's token."""
...
def _project_repository_fields(data: dict) -> dict:
    ...
    return {
        "repo_id": str(data["id"]),
        "repo_owner": owner or full_name.split("/", 1)[0],
        "repo_name": full_name,
        ...
        "repo_visibility": data.get("visibility") or ("private" if private else "public"),
        "repo_size_kb": int(data.get("size") or 0),
    }


def _project_query(db: Session, project_id: str, user_id: str):
    return (
        db.query(Project)
        .options(
...
def get_projects(
    ...
    return (
        db.query(Project)
        .options(selectinload(Project.targets), selectinload(Project.scans))
...
async def create_project(
    ...
    """Create a project and store its single repository connection atomically."""
    try:
        if not project_in.repository:
            raise HTTPException(status_code=400, detail="A GitHub repository is required")

        verified_data = await _fetch_github_repository(current_user, project_in.repository)
        repo_fields = _project_repository_fields(verified_data)

        project_name = (project_in.name or repo_fields["repo_name"].split("/")[-1]).strip()
        if not project_name:
            raise HTTPException(status_code=400, detail="Project name cannot be empty")

        branch = (project_in.branch or repo_fields["default_branch"] or "main").strip()
        if not branch:
        ...
        db.commit()
        return _project_query(db, project.id, current_user.id).first()
        ...
        db.rollback()
        raise HTTPException(status_code=409, detail="The project could not be created because of a database constraint") from exc
    except Exception:
        db.rollback()
        raise
...
def get_project(
    ...
    project = _project_query(db, project_id, current_user.id).first()
    if not project:
...
def update_project(
    ...
    if "verified" in values:
        project.verified = values["verified"]

    try:
        db.commit()
        return _project_query(db, project_id, current_user.id).first()
    except Exception:
        db.rollback()
        raise
